...
export const showProcessingToast = (message: string): (() => void) => {
  const toast = document.createElement('div')
  toast.className = `
    absolute bottom-0 right-0 z-10 max-w-xs w-auto
    bg-yellow-400 text-black
    px-3 py-2 rounded-none shadow-[4px_4px_0px_0px_#000]
  ...
  `
  toast.innerHTML = `
    <svg class="w-4 h-4 animate-spin text-black" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const actionsRight = document.getElementById('editor-actions-right')
  if (actionsRight) {
    // Position inside the actions right container
    actionsRight.style.position = actionsRight.style.position || 'relative'
    actionsRight.appendChild(toast)
  } else {
    // Fallback to body bottom-right if container not found
