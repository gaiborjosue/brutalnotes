...
import React, { useState, useEffect, useRef, createContext, useContext, type ReactNode, useCallback, useMemo } from 'react'
...
const GlobalTourPopover: React.FC = () => {
  ...
  return (
    <div
      ref={popoverRef}
      className="w-96 max-w-[90vw] transition-all duration-300 ease-out font-mono"
      style={popoverStyle}
    >
      <Card className="border-4 border-black shadow-[8px_8px_0px_0px_#000] bg-white">
        <CardHeader className="border-b-4 border-black bg-yellow-300 pb-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 border-2 border-black bg-black text-white text-sm flex items-center justify-center font-black shadow-[2px_2px_0px_0px_#666]">
                {currentStepIndex + 1}
              </div>
              <CardTitle className="text-xl font-black text-black uppercase tracking-wide">{currentStepData.title}</CardTitle>
            </div>
            <Button
              variant="neutral"
              size="icon"
              onClick={stopTour}
              className="h-8 w-8 border-2 border-black shadow-[2px_2px_0px_0px_#000] bg-red-400 hover:bg-red-500 text-black font-black"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
          <div className="w-full h-3 border-2 border-black bg-white shadow-[2px_2px_0px_0px_#000]">
            <div
        ...
        <CardContent className="pt-4 pb-4 px-4">
          <CardDescription className="text-sm leading-relaxed mb-6 text-black font-bold">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-2 justify-end flex-wrap">
              {!isFirstStep && (
                <Button 
                  ...
                  size="sm" 
                  onClick={prevStep}
                  className="border-2 border-black shadow-[2px_2px_0px_0px_#000] bg-gray-300 hover:bg-gray-400 text-black font-black h-9 px-4"
                >
              ...
              <Button 
                size="sm" 
                onClick={nextStep}
                className="border-2 border-black shadow-[2px_2px_0px_0px_#000] bg-blue-400 hover:bg-blue-500 text-black font-black h-9 px-4"
              >
            ...
            <Button
              ...
              size="sm"
              onClick={stopTour}
              className="border-2 border-black shadow-[2px_2px_0px_0px_#000] bg-orange-300 hover:bg-orange-400 text-black font-black h-8 text-xs"
            >
              SKIP TOUR
            </Button>
          </div>
...
const TourStepComponent: React.FC<{
  ...
  return (
    <div
      ...
      className={`${isCurrentStep ? "relative z-10002" : "relative"} h-full w-full`}
    >
      {children}
    </div>
  )
